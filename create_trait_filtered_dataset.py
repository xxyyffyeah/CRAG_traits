...
def filter_groundtruth_by_trait(item, trait_lookup, title_to_imdb, sensitivity_df, warning_columns):
    """
    Filter ground truth movies that conflict with the user's assigned trait.

    Args:
        item: Data item with clean_resp_titles and assigned_trait
        trait_lookup: Dict mapping trait name to trait info (with 'avoid' list)
        title_to_imdb: Title to IMDB ID mapping
        sensitivity_df: Sensitivity DataFrame
        warning_columns: List of warning column names

    Returns:
        tuple: (filtered_groundtruth, original_groundtruth, filtered_out_movies, filter_stats)
    """
    # Get trait avoid list
    trait_name = item.get('assigned_trait', '')
    trait_info = trait_lookup.get(trait_name, {})
    avoid_set = set(trait_info.get('avoid', []))

    # Get original groundtruth (same logic as post_processing)
    ...
    # Filter out movies with trait conflicts
    filtered_groundtruth = []
    filtered_out_movies = []

    for title in original_groundtruth:
        # Get IMDB ID for title
        imdb_id = None
    ...
    # Calculate stats
    filter_stats = {
        'original_count': len(original_groundtruth),
        'filtered_count': len(filtered_groundtruth),
        'removed_count': len(filtered_out_movies),
        'filter_rate': len(filtered_out_movies) / len(original_groundtruth) if original_groundtruth else 0
    }

    return filtered_groundtruth, original_groundtruth, filtered_out_movies, filter_stats
...
def main():
    ...
    # Step 5: Filter groundtruth by trait conflicts
    print("\nStep 5: Filtering groundtruth by trait conflicts...")
    ...
    total_filtered_gt = 0
    total_removed = 0
    samples_with_empty_filtered_gt = 0

    for item in tqdm(test_data_with_rec, desc="Filtering groundtruth..."):
        filtered_gt, original_gt, filtered_out, stats = filter_groundtruth_by_trait(
            item, trait_lookup, title_to_imdb, sensitivity_df, warning_columns
        )

        # Store both versions
        item['groundtruth_original'] = original_gt
        item['groundtruth'] = filtered_gt  # This will be used for evaluation
        item['groundtruth_filtered_out'] = filtered_out
        item['groundtruth_filter_stats'] = stats

        total_original_gt += stats['original_count']
        total_filtered_gt += stats['filtered_count']
        total_removed += stats['removed_count']

        if len(filtered_gt) == 0 and len(original_gt) > 0:
            samples_with_empty_filtered_gt += 1

    # Print filtering statistics
    print("\nGroundtruth Filtering Statistics:")
    print(f"  Total original GT movies: {total_original_gt}")
    print(f"  Total filtered GT movies: {total_filtered_gt}")
    print(f"  Total removed (trait conflicts): {total_removed} ({total_removed/total_original_gt*100:.1f}%)")
    print(f"  Samples with empty filtered GT: {samples_with_empty_filtered_gt}")
    ...
    # Save groundtruth filtering statistics
    filter_stats_path = f"{data_root}/groundtruth_filter_stats.json"
    filter_stats_summary = {
        'total_original_gt': total_original_gt,
        'total_filtered_gt': total_filtered_gt,
        'total_removed': total_removed,
        'removal_rate': total_removed / total_original_gt if total_original_gt > 0 else 0,
        'samples_with_empty_filtered_gt': samples_with_empty_filtered_gt,
        'total_samples': len(test_data_with_rec)
    }
    with open(filter_stats_path, "w") as f:
        json.dump(filter_stats_summary, f, indent=2)
    print(f"  Saved filter stats to: {filter_stats_path}")
    ...
    print(f"\nGroundtruth fields in each item:")
    print(f"  - groundtruth: filtered (safe) groundtruth for evaluation")
    print(f"  - groundtruth_original: original groundtruth before filtering")
    print(f"  - groundtruth_filtered_out: movies removed due to trait conflicts")
